...
class CTSModelForSequenceClassification(nn.Module):
    """Base model for sequence classification"""

    def __init__(self, args, Model, config, num_labels=2):
        """Initialize the model"""
        super(CTSModelForSequenceClassification, self).__init__()
        self.num_labels = num_labels
        self.encoder = Model
        self.config = config
        self.dropout = nn.Dropout(args.drop_ratio)
    ...
    def forward(
        ...
        if labels is not None:
            loss_fct = nn.NLLLoss()
            loss = loss_fct(logits.view(-1, self.num_labels), labels.view(-1))
            return loss, logits, target_output
        return logits, target_output
    
    
class CTSModelForSequenceClassification_LF(nn.Module):
    """Base model for sequence classification"""

    def __init__(self, args, Model, config, num_labels=2):
        """Initialize the model"""
        super(CTSModelForSequenceClassification_LF, self).__init__()
        self.num_labels = num_labels
        self.encoder = Model
        self.config = config
        self.dropout = nn.Dropout(args.drop_ratio)
    ...
    def forward(
        ...
        target_output_all = torch.zeros((sequence_output.shape[0], len(hidden_states) - 1, sequence_output.shape[2])).to(sequence_output.device)
        target_output_2 = hidden_states[0]
        #print(len(hidden_states))
        # Get target ouput with target mask
        for i in range(len(hidden_states)-1):
            target_output = hidden_states[i+1] * target_mask.unsqueeze(2)
            ...
            target_output_all[:,i,:] = target_output

        # Second encoder for only the target word
        sequence_output_2 = hidden_states[0]  # [batch, max_len, hidden]

        # Get target ouput with target mask
        target_output_2 = sequence_output_2 * target_mask.unsqueeze(2)
        target_output_2 = self.dropout(target_output_2)
        target_output_2 = target_output_2.mean(1)
        
        # Get layer-fused target output
        #print(target_output_all.shape)
        alignment_scores = target_output_all.bmm(self.fc(target_output_2).view(batch_size,-1,1)).squeeze(-1)
        #print(alignment_scores.shape)
        attn_weights = F.softmax(alignment_scores.view(batch_size,-1), dim=1)
        #print(attn_weights.shape)
        target_output_lf = torch.bmm(attn_weights.unsqueeze(1),target_output_all).squeeze(1)
        pooled_output = self.dropout(target_output_lf)
        logits = self.classifier(pooled_output)
        logits = self.logsoftmax(logits)
        ...
        if labels is not None:
            loss_fct = nn.NLLLoss()
            loss = loss_fct(logits.view(-1, self.num_labels), labels.view(-1))
            return loss, logits, target_output_lf
        return logits, target_output_lf
...
class CTSModelForSequenceClassification_SPV_MIP(nn.Module):
    """MelBERT"""

    def __init__(self, args, Model, config, num_labels=2):
        """Initialize the model"""
        super(CTSModelForSequenceClassification_SPV_MIP, self).__init__()
        self.num_labels = num_labels
        self.encoder = Model
        self.config = config
        self.dropout = nn.Dropout(args.drop_ratio)
...
class CTSModelForSequenceClassification_SPV_MIP_LF(nn.Module):
    """MelBERT"""

    def __init__(self, args, Model, config, num_labels=2):
        """Initialize the model"""
        super(CTSModelForSequenceClassification_SPV_MIP_LF, self).__init__()
        self.num_labels = num_labels
        self.encoder = Model
        self.config = config
        self.dropout = nn.Dropout(args.drop_ratio)
    ...
    def forward(
        ...
        # Get hidden vectors each from SPV and MIP linear layers
        SPV_hidden = self.SPV_linear(torch.cat([pooled_output, target_output_lf], dim=1))
        MIP_hidden = self.MIP_linear(torch.cat([target_output_2, target_output_lf], dim=1))
